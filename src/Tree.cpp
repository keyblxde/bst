...
// Returns the node with smallest key greater than provided node
TreeNode* Tree::findSuccessor(TreeNode* node) {
    if (node->right != nullptr) {
        return findMin(node->right);
    }

    // Working way back up tree till we reach successor
...
// Returns the node with the greatest key smaller than provided node
